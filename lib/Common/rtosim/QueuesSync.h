...
#ifndef ceinms_QueuesSync_h
#define ceinms_QueuesSync_h
#include <tuple>
#include <limits>
...
namespace ceinms {
    namespace QueuesSync {
        ...
        template<typename T>
        bool isEndOfData(const T& frame) {
            return frame.time == std::numeric_limits<double>::infinity();
        }
        ...
        typename T::type syncToTime(double t, T& queue) {
            ...
            do {
                frame = queue.pop();
            } while (frame.time < t && !isEndOfData(frame));
            return frame;
        }
